...
- 修仙帮助、修仙百科、后台接口、玩家、背包、纳戒、保险箱、修仙物品、银行、二手市场、贸易服务、宗门、探险、武器、装备、铭刻、对战、异界虫洞、首领、修仙界历史、世界皮肤、数据库备份都是中文二级组件。
- HTTP 路由由 `后台接口` 和 `修仙帮助` 承接；`后台接口` 是 web 后台 API 承接点，`修仙帮助` 提供 Hexo/Vexos 风格 Markdown 文档站。
- 数据库使用 sqlite3，schema 版本为 `SCHEMA_VERSION = 2026062309`；系统 NPC 点、跑商城池、探险地点、特殊收购点、回收点、城池状态、战备状态和交易动态状态都以稳定 ID 承接，中文名只作为当前世界皮肤的展示名；`player_weapons` 保存累计经验，但不保存攻击、自带技能或附魔槽位这类可实时派生字段；`sects` 保存宗门山门坐标、创建者和宗主，`sect_members` 保存宗门成员归属，`sect_stats` 保存宗门等级和三底蕴，宗门大会数据按周期累计到宗门影响力、个人贡献和奖励表。
...
- 修仙物品设定分成世界物资和角色成长道具。世界物资走纯经济、药路、民生、建设、古物、战利品六大类；武器、技能书、宝石、恢复药、开孔器、洗髓液、淬锋丹、铭刻之羽和原石不进入六大类。
...
- 库存链路：探险、跑商、抢劫、首领、虫洞和藏宝图产出物品；背包/纳戒/武器库/装备/银行分别承接，保险箱提供冻结保护，贸易服务和二手市场处理流转。
