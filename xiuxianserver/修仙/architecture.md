...
- 修仙帮助、修仙百科、后台接口、玩家、背包、纳戒、保险箱、修仙物品、源库、二手市场、商场、宗门、探险、武器、装备、铭刻、对战、异界虫洞、首领、修仙界历史、数据库备份都是中文二级组件。
- HTTP 路由由 `后台接口` 和 `修仙帮助` 承接；`后台接口` 是 web 后台 API 承接点，`修仙帮助` 提供 Hexo/Vexos 风格 Markdown 文档站。
- 数据库使用 sqlite3，schema 版本为 `SCHEMA_VERSION = 2026061705`；`player_weapons` 保存累计经验，但不保存攻击、自带技能或附魔槽位这类可实时派生字段；`sects` 保存宗门山门坐标、创建者和宗主，`sect_members` 保存宗门成员归属，宗门战数据按周期累计到宗门影响力、个人贡献和奖励表。
...
- 二级目录统一使用中文，例如 `修仙帮助`、`后台接口`、`玩家`、`源库`、`纳戒`、`商场`。
...
12. 修仙文本回复都会经 `send_reply` 转成 markdown，并自动补玩家头、预测按钮、当前组件按钮和默认按钮；手写 `<休息>` 这类标记会优先转成业务按钮。
...
- 回复层不从带占位参数的建议文案里猜按钮；没有 `<命令>` 标记时，也会因为预测按钮和默认按钮统一作为 `markdown` 发送。
...
- 预测按钮来自回复正文关键词，只生成固定命令，不自动生成需要用户补参数的命令。
- 当前组件按钮来自 `reply.py` 的 `CONTEXT_BUTTONS_BY_GROUP`，只在自动补齐阶段使用。
- 最终顺序是业务手写按钮、预测按钮、当前组件按钮、默认按钮，并按顺序去重。
...
world_locations             命名世界点位，按坐标主键稀疏保存
sects                       宗门山门、宗主和创建信息
...
修仙世界当前按矩形整数坐标设计，左下角为 `(-100,-100)`，右上角为 `(100,100)`，边界点可用；数据库只保存商场、探险点、NPC 建筑、回收建筑等命名点，荒野空地不入库，洞府、地皮和藏宝图按坐标稀疏占用。
