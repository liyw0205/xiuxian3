...
这里维护岁时情劫自己的首领文案、节令映射和战斗技能。
它不属于世界皮肤包：岁时情劫和现实节令绑定，允许在本文件内改数值和文案，
但不跟随世界物品、地点、怪物的热插拔命名一起变化。
"""
...
SEASONAL_BOSS_KIND = "岁时旧愿"

SEASONAL_SKILL_TEMPLATES: dict[str, dict[str, Any]] = {
    "old_vow": {"skill_key": "seasonal_skill_old_vow", "name": "旧愿缠心", "interval": 4, "power": 1.08, "effects": {"mp_suppress": 0.07}},
    "parting_wind": {"skill_key": "seasonal_skill_parting_wind", "name": "离风照影", "interval": 4, "power": 1.10, "effects": {"stun_rate": 0.05}},
    "broken_oath": {"skill_key": "seasonal_skill_broken_oath", "name": "残誓回潮", "interval": 5, "power": 1.15, "effects": {"bleed_rate": 0.06}},
    "cold_memory": {"skill_key": "seasonal_skill_cold_memory", "name": "霜念封身", "interval": 5, "power": 1.12, "effects": {"damage_reduce": 0.06}},
    "burning_lantern": {"skill_key": "seasonal_skill_burning_lantern", "name": "孤灯余烬", "interval": 5, "power": 1.14, "effects": {"burn_rate": 0.06}},
}
SEASONAL_SKILL_PROFILE_ROTATION = ("old_vow", "parting_wind", "broken_oath", "cold_memory", "burning_lantern")
SEASONAL_SKILL_PROFILE_OVERRIDES: dict[str, str] = {
    "lichun": "parting_wind",
    "qingming": "cold_memory",
    "duanwu": "broken_oath",
    "qixi": "old_vow",
    "zhongqiu": "cold_memory",
    "chongyang": "parting_wind",
    "chunjie": "burning_lantern",
    "yuanxiao": "burning_lantern",
}


def seasonal_skill_for_event(event: dict[str, Any]) -> dict[str, Any]:
    """按岁时事件生成专属首领技能。"""

    key = str(event.get("boss_key") or event.get("boss_name") or "")
    level = int(event.get("level") or 1)
    return seasonal_skill_for_key(key, level)


def seasonal_skill_for_key(key: str, level: int) -> dict[str, Any]:
    """生成岁时首领技能副本，避免战斗过程修改模板。"""

    profile = _seasonal_skill_profile(key)
    template = SEASONAL_SKILL_TEMPLATES.get(profile, SEASONAL_SKILL_TEMPLATES["old_vow"])
    level_bonus = min(0.12, max(1, int(level)) / 900)
    return {
        "skill_key": str(template["skill_key"]),
        "name": str(template["name"]),
        "cost_mp": 0,
        "interval": max(3, int(template.get("interval", 5))),
        "power": float(template.get("power", 1.1)) + level_bonus,
        "effects": dict(template.get("effects") or {}),
    }


def _seasonal_skill_profile(key: str) -> str:
    """按首领 key 稳定挑选技能模板；特殊节日可显式覆盖。"""

    key = str(key or "")
    override = SEASONAL_SKILL_PROFILE_OVERRIDES.get(key)
    if override:
        return override
    return SEASONAL_SKILL_PROFILE_ROTATION[sum(ord(char) for char in key) % len(SEASONAL_SKILL_PROFILE_ROTATION)]
