...
class BackpackService(CoreService):
    ...
    def use_item(self, client_id: str, item_name: str) -> str:
        ...
        if not item:
            ...
            if not ring_item:
                return hint(f"没有找到物品：{item_name.strip()}。", "发送：背包 或 纳戒，复制准确物品名。<背包><纳戒>")
            if ring_item["category"] != "恢复类":
                return hint(f"{ring_item['name']} 不能直接使用。", equipment_item_use_hint(ring_item))
            with self.db.transaction() as conn:
                if not self.remove_ring_conn(conn, client_id, ring_item["equipment_item_id"], 1):
                    return hint(f"纳戒里没有 {ring_item['name']}。", "发送：纳戒 确认库存，或继续探险获取。<纳戒>")
                return item_effects.apply_conn(conn, client_id, ring_item, "纳戒")
        if item["category"] != "恢复类":
            return hint(f"{item['name']} 不能直接使用。", "跑商物品可发送：商场出售 商品名 数量；特殊战利品可发送：特殊出售 物品名 数量。<商场自动出售><特殊自动出售>")

        with self.db.transaction() as conn:
            if not self.remove_backpack_conn(conn, client_id, item["item_id"], 1):
                return hint(f"背包里没有 {item['name']}。", "发送：背包 确认库存，或继续探险/跑商获取。<背包><商场推荐>")
