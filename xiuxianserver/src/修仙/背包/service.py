...
from ..common import CoreService, hint
...
class BackpackService(CoreService):
    ...
    def use_item(self, client_id: str, item_name: str) -> str:
        ...
        if not item:
            ...
            if not ring_item:
                return hint(f"没有找到物品：{item_name.strip()}。", "发送：背包 或 纳戒，复制准确物品名。")
            ...
            with self.db.transaction() as conn:
                if not self.remove_ring_conn(conn, client_id, ring_item["equipment_item_id"], 1):
                    return hint(f"纳戒里没有 {ring_item['name']}。", "发送：纳戒 确认库存，或继续探险获取。")
        ...
        with self.db.transaction() as conn:
            if not self.remove_backpack_conn(conn, client_id, item["item_id"], 1):
                return hint(f"背包里没有 {item['name']}。", "发送：背包 确认库存，或继续探险/跑商获取。")
