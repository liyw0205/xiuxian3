...
@WsMessageHandler.handler(cmd=("商场", "跑商"), priority=100, block=True)
...
@WsMessageHandler.handler(cmd=("商场列表", "列表商场", "跑商列表"), priority=100, block=True)
...
@WsMessageHandler.handler(cmd=("商场详情", "详情商场", "跑商详情"), priority=100, block=True)
...
@WsMessageHandler.handler(cmd=("商场市价", "市价商场", "跑商市价"), priority=100, block=True)
...
@WsMessageHandler.handler(cmd=("商场购买", "购买商场", "跑商购买"), priority=100, block=True)
...
@WsMessageHandler.handler(cmd=("商场出售", "出售商场", "跑商出售"), priority=100, block=True)
...
@WsMessageHandler.handler(cmd=("商场自动出售", "自动出售商场", "跑商自动出售"), priority=100, block=True)
...
@WsMessageHandler.handler(cmd=("商场推荐", "推荐商场", "跑商推荐"), priority=100, block=True)
...
@WsMessageHandler.handler(cmd=("商场记录", "记录商场", "跑商记录"), priority=100, block=True)
...
@WsMessageHandler.handler(cmd=("商场限制", "限制商场", "跑商限制"), priority=100, block=True)
...
@WsMessageHandler.handler(cmd=("商场奖励", "奖励商场", "跑商奖励", "奖励跑商"), priority=100, block=True)
async def ws_trade_daily_reward(client_id: str, message: str) -> None:
    """领取每日跑商奖励。"""

    await ws_manager.send(service.daily_reward(client_id), client_id)


@WsMessageHandler.handler(cmd=("特殊收购", "收购特殊", "收购"), priority=100, block=True)
...
@WsMessageHandler.handler(cmd=("特殊出售", "出售特殊", "战利品出售"), priority=100, block=True)
...
@WsMessageHandler.handler(cmd=("特殊自动出售", "自动出售特殊", "自动出售战利品"), priority=100, block=True)
...
@WsMessageHandler.handler(cmd=("导航", "去", "前往"), priority=100, block=True)
